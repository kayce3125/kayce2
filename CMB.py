...
header_grup = {"user-agent": "Mozilla/5.0 (Linux; U; Android 1.1; en-us; dream) AppleWebKit/525.10  (KHTML, like Gecko) Version/3.0.4 Mobile Safari/523.12.2"}
...
def moch_yayan():
    ...
    try:
        ...
        print("\n %s[%s×%s] Lisense invalid"%(N,M,N));time.sleep(2);cek_key()
    ...
    print(' [%s01%s]. Crack id dari anggota grup'%(O,H));time.sleep(0.03)
    print(' [%s02%s]. Crack id dari teman publik'%(O,H));time.sleep(0.03)
    print(' [%s03%s]. Crack id dari total followers'%(O,H));time.sleep(0.03)
    print(' [%s04%s]. Crack id dari like postingan'%(O,H));time.sleep(0.03)
    print(' [%s05%s]. Crack id dari random id massal'%(O,H));time.sleep(0.03)
    print(' [%s06%s]. Crack id dari komentar postingan'%(O,H));time.sleep(0.03)
    print(' [%s07%s]. Checkpoint detedtor'%(O,H));time.sleep(0.03)
    print(' [%s08%s]. Settings user agent'%(O,H));time.sleep(0.03)
    print(' [%s09%s]. Check hasil crack'%(O,H));time.sleep(0.03)
...
def cok():
    ...
    print("[*] THE SCRIPT HAS BEEN UPTED ON [Monday 12 February 2022]")
    key = input("\n[*] enter your api key : ")
    if key == '':
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    print(' [%s1%s]. METHOD MBASIC (SLOW)'%(O,K))
                    print(' [%s2%s]. METHOD MOBILE (FAST)'%(O,K))
                    print(' [%s3%s]. METHOD API (FASTER) [\033[92mRecommended\033[0m]'%(O,N))
                    __yan__(pwek.split(','))
                    break
        elif ___yayanganteng___ in ('T', 't'):
            print('\n [ pilih method login - silahkan coba satu² ]\n')
            print(' [%s1%s]. METHOD MBASIC (SLOW)'%(O,N))
            print(' [%s2%s]. METHOD MOBILE (FAST)'%(O,N))
            print(' [%s3%s]. METHOD API (FASTER) [\033[92mDisarankan\033[0m]'%(O,N))
            self.__pler__()
        else:
